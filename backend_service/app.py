...
def responder_heartbeat():
    ...
    socketio.emit(
        ...
        {"responder_user_id": responder_user_id, "district_id": district_id, "status": "ONLINE"},
        room=district_id
    )
    return jsonify({"status": "heartbeat_received", "responder": responders[responder_user_id]})
...
def responder_location():
    ...
    trails[responder_user_id].append({"lat": lat, "lon": lon, "timestamp": time.time()})
    fake_lat = lat + random.uniform(-0.002, 0.002)
    fake_lon = lon + random.uniform(-0.002, 0.002)
    socketio.emit(
        EVENT_RESPONDER_LOCATION_UPDATE,
        {"responder_user_id": responder_user_id, "district_id": district_id,
         "lat": fake_lat, "lon": fake_lon, "status": "ONLINE"},
        room=district_id
    )
    return jsonify({
        "status": "location_updated",
        "responder": responders[responder_user_id],
        "public_location": {"lat": fake_lat, "lon": fake_lon}
    })
...
def get_responders():
    ...
    if district_id:
        result = [r for r in result if r.get("district_id") == district_id]
...
def critical_incident():
    payload = request.get_json(force=True)
    district_id = payload.get("district_id", "NATIONAL")
    socketio.emit(EVENT_CRITICAL_INCIDENT, payload, room=district_id)
    return jsonify({"status": "sent", "event": EVENT_CRITICAL_INCIDENT, "district_id": district_id})
